...
/datum/disease/pierrot_throat/stage_act()
	...
	switch(stage)
		if(1)
			if(prob(10))
				to_chat(affected_mob, span_danger("I feel a little silly."))
		if(2)
			if(prob(10))
				to_chat(affected_mob, span_danger("I start seeing rainbows."))
		if(3)
			if(prob(10))
				to_chat(affected_mob, span_danger("My thoughts are interrupted by a loud <b>HONK!</b>"))
		if(4)
			if(prob(5))
