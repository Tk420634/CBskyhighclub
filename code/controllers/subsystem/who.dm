...
/datum/controller/subsystem/who/proc/Me(client/whoer)
	...
	dat += "<hr>"
	dat += span_green("I am <a href='?src=[REF(src)];[whoer.ckey]=SetCustomName'>[myname]</a> the <a href='?src=[REF(src)];[whoer.ckey]=SetCustomJob'>[myrole]</a>!<br>")
	dat += span_notice("My name is <a href='?src=[REF(src)];[whoer.ckey]=ToggleNameVis'>[shows_name ? "VISIBLE to" : "HIDDEN from"] others!</a><br>")
	dat += span_notice("My role is <a href='?src=[REF(src)];[whoer.ckey]=ToggleRoleVis'>[shows_job ? "VISIBLE to" : "HIDDEN from"] others!</a><br>")
	if(show_coords)
		dat += span_green("I am in [mywhere], within [fuzzy_x], [fuzzy_y] ±[coord_fuzziness]!<br>")
	else
		dat += span_green("I am somewhere in <a href='?src=[REF(src)];[whoer.ckey]=SetCustomLocation'>[mywhere]</a>!<br>")
	dat += span_notice("My current location is <a href='?src=[REF(src)];[whoer.ckey]=ToggleLocationVis'>[shows_location ? "VISIBLE to" : "HIDDEN from"] others!</a><br>")
	if(show_coords)
		dat += span_notice("My current Coordinates are <a href='?src=[REF(src)];[whoer.ckey]=ToggleLocationCoordVis'>[shows_coords ? "VISIBLE to" : "HIDDEN from"] others!</a><br>")
	dat += span_notice("My OOC status is: '[mypose]' <a href='?src=[REF(src)];[whoer.ckey]=SetPose'>\[Change?\]</a><br>")
	dat += span_notice("My OOC status is <a href='?src=[REF(src)];[whoer.ckey]=TogglePoseVis'>[shows_pose ? "VISIBLE to" : "HIDDEN from"] others!</a><br>")
	dat += span_brass("I am currently <a href='?src=[REF(src)];[whoer.ckey]=ToggleWhoVis'>[shows_who ? "VISIBLE" : "HIDDEN"] on the who list!</a><br>")
	if(is_admin)
		dat += "I am \a [whoer.holder.rank]"
...
/datum/controller/subsystem/who/Topic(href, list/href_list)
	...
	switch(href_list["[ckey]"])
		if("ToggleLocationVis")
			...
			to_chat(P.parent, span_green("My location is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_WHERE) ? "VISIBLE" : "HIDDEN"] to others!"))
		if("ToggleWhoVis")
			TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_ME)
			to_chat(P.parent, span_green("I am now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_ME) ? "VISIBLE" : "HIDDEN"] to others!"))
		if("ToggleNameVis")
			TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_NAME)
			to_chat(P.parent, span_green("My name is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_NAME) ? "VISIBLE" : "HIDDEN"] to others!"))
		if("ToggleRoleVis")
			TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_ROLE)
			to_chat(P.parent, span_green("My role is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_ROLE) ? "VISIBLE" : "HIDDEN"] to others!"))
		if("ToggleLocationCoordVis")
			TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_COORDS)
			to_chat(P.parent, span_green("My coordinates are now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_COORDS) ? "VISIBLE" : "HIDDEN"] to others!"))
		if("TogglePoseVis")
			TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_POSE)
			to_chat(P.parent, span_green("My OOC status is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_POSE) ? "VISIBLE" : "HIDDEN"] to others!"))	
		if("SetPose")
			SetPose(M)
		if("SetCustomName")
			SetName(M)
...
/datum/who_custom_stuff/proc/DispenseInfo(oldval, newval, key, cleared)
	...
	if(cleared)
		if(is_admin)
			...
			to_chat(usr, "I cleared [M]'s [key] (Was:[oldval])")
			to_chat(M, span_userdanger("My [key] has been cleared by an admin! (Was:[oldval])"))
		else
			to_chat(usr, "I cleared your [key] (Was:[oldval])")
			to_chat(M, "My [key] has been cleared (Was:[oldval])")
	else
		if(is_admin)
			message_admins("[usr.ckey] set [M]'s ([ckey]) [key] to: [newval] (Was:[oldval])")
			log_ooc("[usr.ckey] set [M]'s ([ckey]) [key] to: [newval] (Was:[oldval])")
			to_chat(usr, "I set [M]'s [key] to: [newval] (Was: [oldval])")
			to_chat(M, span_userdanger("My [key] has been changed by an admin! Now: [newval] (Was: [oldval])"))
		else
			to_chat(usr, "I set your [key] to: [newval] (Was: [oldval])")
			to_chat(M, "My [key] has been set to: [newval] (Was: [oldval])")
...
/datum/who_custom_stuff/proc/ToggleNameVis()
	...
	var/datum/preferences/P = extract_prefs(ckey)
	TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_NAME)
	to_chat(M, "My name is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_NAME) ? "VISIBLE" : "HIDDEN"] to others!")

/datum/who_custom_stuff/proc/ToggleRoleVis()
	var/mob/M = GetMyMob()
	var/datum/preferences/P = extract_prefs(ckey)
	TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_ROLE)
	to_chat(M, "My role is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_ROLE) ? "VISIBLE" : "HIDDEN"] to others!")

/datum/who_custom_stuff/proc/ToggleLocationVis()
	var/mob/M = GetMyMob()
	var/datum/preferences/P = extract_prefs(ckey)
	TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_WHERE)
	to_chat(M, "My location is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_WHERE) ? "VISIBLE" : "HIDDEN"] to others!")

/datum/who_custom_stuff/proc/TogglePoseVis()
	var/mob/M = GetMyMob()
	var/datum/preferences/P = extract_prefs(ckey)
	TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_POSE)
	to_chat(M, "My OOC status is now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_POSE) ? "VISIBLE" : "HIDDEN"] to others!")

/datum/who_custom_stuff/proc/ToggleWhoVis()
	var/mob/M = GetMyMob()
	var/datum/preferences/P = extract_prefs(ckey)
	TOGGLE_BITFIELD(P.whoflags, WHO_SHOWS_ME)
	to_chat(M, "I am now [CHECK_BITFIELD(P.whoflags, WHO_SHOWS_ME) ? "VISIBLE to" : "HIDDEN from"] the Who list!")
...
/datum/who_custom_stuff/proc/ToggleLockout(lock_key)
	...
	to_chat(usr, "I have [was_on ? span_alert("forbidden") : span_green("allowed")] [M.real_name] ([ckey]) to change their Custom [lock_key]!")
	/// The person who was locked out
	if(was_on)
		to_chat(M, span_alert("I am no longer allowed to change your Custom [lock_key]!"))
	else
		to_chat(M, span_green("I am now allowed to change your Custom [lock_key]!"))
