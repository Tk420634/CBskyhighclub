...
/mob/living/simple_animal/parrot
	...
	turns_per_move = 5
	guaranteed_butcher_results = list(/obj/item/reagent_containers/food/snacks/cracker/ = 1)
				...
				if(ears)
					...
					for(var/ch in headset_to_add.channels)
						switch(ch)
							...
							if(RADIO_CHANNEL_RECLAIMER)
								available_channels.Add(RADIO_TOKEN_RECLAIMER)
		...
		if(Adjacent(parrot_interest))
			...
			if(isliving(parrot_interest))
				...
				if(!parrot_perch || parrot_interest.loc != parrot_perch.loc)
					...
					visible_message("[src] grabs [held_item]!", span_notice("I grab [held_item]!"), span_italic("I hear the sounds of wings flapping furiously."))
...
/mob/living/simple_animal/parrot/proc/steal_from_mob()
	...
	for(var/mob/living/carbon/C in view(1,src))
		...
		if(stolen_item)
			...
			visible_message("[src] grabs [held_item] out of [C]'s hand!", span_notice("I snag [held_item] out of [C]'s hand!"), span_italic("I hear the sounds of wings flapping furiously."))
			return held_item
...
/mob/living/simple_animal/parrot/proc/drop_held_item(drop_gently = 1)
	...
	if(!held_item)
		if(src == usr) //So that other mobs wont make this message appear when they're bludgeoning you.
			to_chat(src, span_danger("I have nothing to drop!"))
		return 0
...
/mob/living/simple_animal/parrot/proc/toggle_mode()
	...
	if(a_intent != INTENT_HELP)
		...
		melee_damage_upper = parrot_damage_upper
		a_intent = INTENT_HARM
	to_chat(src, "I will now [a_intent] others.")
	return
...
/mob/living/simple_animal/parrot/Poly
	...
	speak = list("Poly wanna cracker!", ":e Check the crystal, you chucklefucks!",":e Wire the solars, you lazy bums!",":e WHO TOOK THE DAMN HARDSUITS?",":e OH FUCK ITS ABOUT TO DELAMINATE CALL THE SHUTTLE")
