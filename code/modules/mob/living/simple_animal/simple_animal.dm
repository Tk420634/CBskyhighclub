...
/mob/living/simple_animal/proc/make_ghostable(mob/user)
	...
	if(istype(user))
		...
		if(user.mind)
			user.mind.store_memory("I was revived by [user.real_name], and thus are compelled to follow their commands and protect them!")
		show_message(span_userdanger("I was revived by [user.real_name], and are bound to protect them and follow their commands!"))
		LAZYREMOVE(GLOB.mob_spawners[initial(name)], src)
		if(!LAZYLEN(GLOB.mob_spawners[initial(name)]))
			GLOB.mob_spawners -= initial(name)
