...
/mob/living/simple_animal/hostile/asteroid/elite/herald/say(message, bubble_type, list/spans = list(), sanitize = TRUE, datum/language/language = null, ignore_spam = FALSE, forced = null, only_overhead)
	. = ..()
	if(.)
...
/datum/action/innate/elite_attack/herald_trishot
	...
	chosen_message = span_boldwarning("I am now firing three shots in your chosen direction.")
	chosen_attack_num = HERALD_TRISHOT

/datum/action/innate/elite_attack/herald_directionalshot
	name = "Circular Shot"
	button_icon_state = "herald_directionalshot"
	chosen_message = span_boldwarning("I am firing projectiles in all directions.")
	chosen_attack_num = HERALD_DIRECTIONALSHOT

/datum/action/innate/elite_attack/herald_teleshot
	name = "Teleport Shot"
	button_icon_state = "herald_teleshot"
	chosen_message = span_boldwarning("I will now fire a shot which teleports you where it lands.")
...
/mob/living/simple_animal/hostile/asteroid/elite/herald/OpenFire()
	if(client)
		switch(chosen_attack)
			if(HERALD_TRISHOT)
				herald_trishot(get_target())
				if(my_mirror != null)
					my_mirror.herald_trishot(get_target())
			if(HERALD_DIRECTIONALSHOT)
				herald_directionalshot()
				if(my_mirror != null)
					my_mirror.herald_directionalshot()
			if(HERALD_TELESHOT)
				herald_teleshot(get_target())
				if(my_mirror != null)
					my_mirror.herald_teleshot(get_target())
			if(HERALD_MIRROR)
				herald_mirror()
		return
