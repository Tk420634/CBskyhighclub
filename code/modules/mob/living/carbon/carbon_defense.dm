...
/mob/living/carbon/attack_slime(mob/living/simple_animal/slime/M)
	...
	if(M.powerlevel > 0)
		...
		if(prob(stunprob))
			...
			target_message = span_danger("I have shocked [src]!"))
...
/mob/living/carbon/proc/help_shake_act(mob/living/carbon/M)
	if(on_fire)
		to_chat(M, span_warning("I can't put [p_them()] out with just your bare hands!"))
		return
	...
	if(health >= 0 && !(HAS_TRAIT(src, TRAIT_FAKEDEATH)))
		...
		if(lying)
			...
			M.visible_message(span_notice("[M] shakes [src] trying to get [p_them()] up!"), \
							span_notice("I shake [src] trying to get [p_them()] up!"), target = src,
			...
			M.visible_message( \
				...
				span_notice("I boop [src] on the nose."), target = src,
			...
			M.visible_message(span_notice("[M] gives [src] a pat on the head to make [p_them()] feel better!"), \
						span_notice("I give [src] a pat on the head to make [p_them()] feel better!"), target = src,
		...
		if(friendly_check && HAS_TRAIT(M, TRAIT_FRIENDLY))
			...
			if(HAS_TRAIT(M, TRAIT_DISTANT)) //No mood buff since you're not really liking it.
				M.visible_message(span_warning("[M] glares at [M] as they give them a pat on the head! They seem annoyed..."), \
					span_warning("I give [M] a pat on the head to make [p_them()] feel better! Their eyes shift towards you contemptuously..."))
				//M.add_lust(-5) //Why are you touching me?
				if(prob(5))
					M.visible_message(span_warning("[M] quickly twists [M]\'s arm!"), \
		...
		for(var/obj/item/I in LB.embedded_objects)
			if(!embeds)
				...
				visible_message(span_notice("[src] examines [p_them()]self."), \
					span_notice("I check yourself for shrapnel."))
		...
		if(istype(ears) && (deafen_pwr || damage_pwr))
			...
			if(ears.damage >= 15)
				...
				if(prob(ears.damage - 5))
					to_chat(src, span_userdanger("I can't hear anything!"))
...
/mob/living/carbon/send_item_attack_message(obj/item/I, mob/living/user, hit_area, obj/item/bodypart/hit_bodypart, totitemdamage)
	...
	if(user == src)
		attack_message_local = "I [message_verb] yourself[message_hit_area] with [I][extra_wound_details]"
