...
/datum/antagonist/fugitive/greet(back_story)
	to_chat(owner, span_boldannounce("I am the Fugitive!"))
	backstory = back_story
	switch(backstory)
		if("prisoner")
			to_chat(owner, "<B>I can't believe we managed to break out of a US Government superjail! Sadly though, our work is not done. The emergency teleport at the station logs everyone who uses it, and where they went.</B>")
			to_chat(owner, "<B>It won't be long until CentCom tracks where we've gone off to. I need to work with my fellow escapees to prepare for the troops US Government is sending, I'm not going back.</B>")
		if("cultist")
			...
			to_chat(owner, "<B>Our religion has been repeatedly culled by US Government because it is categorized as an \"Enemy of the Corporation\", whatever that means.</B>")
			to_chat(owner, "<B>Now there are only four of us left, and US Government is coming. When will our god show itself to save us from this hellish station?!</B>")
		if("waldo")
	...
	to_chat(owner, span_boldannounce("I am not an antagonist in that you may kill whomever you please, but you can do anything to avoid capture."))
