...
/obj/item/barriercube/attack_self(mob/user)
	if(locate(/obj/structure/barricade/slime) in get_turf(loc))
		to_chat(user, span_warning("I can't fit more than one barrier in the same space!"))
		return
	to_chat(user, span_notice("I squeeze [src]."))
...
/obj/item/capturedevice/attack(mob/living/M, mob/user)
	...
	if(!isanimal(M))
		...
		return
	if(M.mind)
		to_chat(user, span_notice("I offer the device to [M]."))
		if(alert(M, "Would you like to enter [user]'s capture device?", "Gold Capture Device", "Yes", "No") == "Yes")
			if(user.canUseTopic(src, BE_CLOSE) && user.canUseTopic(M, BE_CLOSE))
				to_chat(user, span_notice("I store [M] in the capture device."))
...
/obj/item/capturedevice/attack_self(mob/user)
	if(contents.len)
		to_chat(user, span_notice("I open the capture device!"))
		release()
	else
