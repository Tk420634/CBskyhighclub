/datum/mafia_role
	var/name = "Assistant"
	var/desc = "I am a crewmember without any special abilities."
...
/datum/mafia_role/proc/greet()
	...
	to_chat(body,span_danger("I am the [name]."))
	to_chat(body,span_danger("[desc]"))
	switch(team)
		if(MAFIA_TEAM_MAFIA)
			to_chat(body,span_danger("I and your co-conspirators win if you outnumber crewmembers."))
		if(MAFIA_TEAM_TOWN)
			to_chat(body,span_danger("I am a crewmember. Find out and lynch the changelings!"))
		if(MAFIA_TEAM_SOLO)
			to_chat(body,span_danger("I am not aligned to town or mafia. Accomplish your own objectives!"))
...
/datum/mafia_role/proc/show_help(clueless)
	...
	switch(team)
		...
		if(MAFIA_TEAM_SOLO)
			...
			team_span = "comradio"
			the = FALSE
	result += span_notice("The <span class='bold'>[name]</span> is aligned with [the ? "the " : ""]<span class='[team_span]'>[team_desc]</span>")
...
/datum/mafia_role/detective
	name = "Detective"
	desc = "I can investigate a single person each night to learn their team."
...
/datum/mafia_role/detective/handle_action(datum/mafia_controller/game,action,datum/mafia_role/target)
	if(!target || target.game_status != MAFIA_ALIVE)
		to_chat(body,span_warning("I can only investigate alive people."))
		return
	to_chat(body,span_warning("I will investigate [target.body.real_name] tonight."))
...
/datum/mafia_role/detective/proc/investigate(datum/mafia_controller/game)
	...
	if(target)
		if(target.detect_immune)
			to_chat(body,span_warning("My investigations reveal that [target.body.real_name] is a true member of the station."))
			...
			to_chat(body,span_warning("My investigations reveal that [target.body.real_name] is [fluff]"))
			add_note("N[game.turn] - [target.body.real_name] - [team_text]")
	current_investigation = null

/datum/mafia_role/psychologist
	name = "Psychologist"
	desc = "I can visit someone ONCE PER GAME to reveal their true role in the morning!"
...
/datum/mafia_role/psychologist/handle_action(datum/mafia_controller/game, action, datum/mafia_role/target)
	. = ..()
	to_chat(body,span_warning("I will reveal [target.body.real_name] tonight."))
...
/datum/mafia_role/psychologist/proc/therapy_reveal(datum/mafia_controller/game)
	...
	if(current_target)
		...
		to_chat(body,span_warning("I have revealed the true nature of the [current_target]!"))
...
/datum/mafia_role/chaplain
	name = "Chaplain"
	desc = "I can communicate with spirits of the dead each night to discover dead crewmember roles."
...
/datum/mafia_role/chaplain/handle_action(datum/mafia_controller/game, action, datum/mafia_role/target)
	to_chat(body,span_warning("I will commune with the spirit of [target.body.real_name] tonight."))
	current_target = target

/datum/mafia_role/chaplain/proc/commune(datum/mafia_controller/game)
	var/datum/mafia_role/target = current_target
	if(target)
		to_chat(body,span_warning("I invoke spirit of [target.body.real_name] and learn their role was <b>[target.name]<b>."))
		add_note("N[game.turn] - [target.body.real_name] - [target.name]")
		current_target = null

/datum/mafia_role/md
	name = "Medical Doctor"
	desc = "I can protect a single person each night from killing."
...
/datum/mafia_role/md/handle_action(datum/mafia_controller/game,action,datum/mafia_role/target)
	if(!target || target.game_status != MAFIA_ALIVE)
		to_chat(body,span_warning("I can only protect alive people."))
		return
	to_chat(body,span_warning("I will protect [target.body.real_name] tonight."))
...
/datum/mafia_role/lawyer
	name = "Lawyer"
	desc = "I can choose a person during the day to provide extensive legal advice to during the night, preventing night actions."
...
/datum/mafia_role/lawyer/handle_action(datum/mafia_controller/game, action, datum/mafia_role/target)
	...
	if(target == current_target)
		current_target = null
		to_chat(body,span_warning("I have decided against blocking anyone tonight."))
	else
		current_target = target
		to_chat(body,span_warning("I will block [target.body.real_name] tonight."))
...
/datum/mafia_role/hop
	...
	desc = "I can reveal yourself once per game, tripling your vote power but becoming unable to be protected!"
...
/datum/mafia_role/mafia/thoughtfeeder/handle_action(datum/mafia_controller/game,action,datum/mafia_role/target)
	to_chat(body,span_warning("I will feast on the memories of [target.body.real_name] tonight."))
	current_investigation = target
...
/datum/mafia_role/traitor/handle_action(datum/mafia_controller/game, action, datum/mafia_role/target)
	...
	to_chat(body,span_warning("I will attempt to kill [target.body.real_name] tonight."))
...
/datum/mafia_role/traitor/proc/try_to_kill(datum/mafia_controller/source)
	...
	if(game_status == MAFIA_ALIVE && target && target.game_status == MAFIA_ALIVE)
		if(!target.kill(source))
			to_chat(body,span_danger("My attempt at killing [target.body] was prevented!"))
...
/datum/mafia_role/nightmare/handle_action(datum/mafia_controller/game, action, datum/mafia_role/target)
	...
	if(target == flicker_target)
		to_chat(body,span_warning("I will do nothing tonight."))
		flicker_target = null
	flicker_target = target
	if(action == "Flicker")
		to_chat(body,span_warning("I will attempt to flicker [target.body.real_name]'s room tonight."))
	else
		to_chat(body,span_danger("I will hunt everyone in a flickering room down tonight."))
...
/datum/mafia_role/nightmare/proc/flicker_or_hunt(datum/mafia_controller/source)
	...
	if(SEND_SIGNAL(src,COMSIG_MAFIA_CAN_PERFORM_ACTION,source,"nightmare actions",flicker_target) & MAFIA_PREVENT_ACTION)
		to_chat(flicker_target.body, span_warning("My actions were prevented!"))
		return
...
/datum/mafia_role/fugitive/handle_action(datum/mafia_controller/game, action, datum/mafia_role/target)
	...
	if(!charges)
		...
		return
	if(game.phase == MAFIA_PHASE_NIGHT)
		to_chat(body,span_danger("I don't have time to prepare, night has already arrived."))
		return
	if(protection_status == FUGITIVE_WILL_PRESERVE)
		to_chat(body,span_danger("I decide to not prepare tonight."))
	else
		to_chat(body,span_danger("I decide to prepare for a horrible night."))
	protection_status = !protection_status

/datum/mafia_role/fugitive/proc/night_start(datum/mafia_controller/game)
	if(protection_status == FUGITIVE_WILL_PRESERVE)
		to_chat(body,span_danger("My preparations are complete. Nothing could kill you tonight!"))
...
/datum/mafia_role/fugitive/proc/night_end(datum/mafia_controller/game)
	if(protection_status == FUGITIVE_WILL_PRESERVE)
		...
		to_chat(body,span_danger("I am no longer protected. You have [charges] use[charges == 1 ? "" : "s"] left of your power."))
...
/datum/mafia_role/obsessed/proc/find_obsession(datum/mafia_controller/game)
	...
	to_chat(body, span_userdanger("My obsession is [obsession.body.real_name]! Get them lynched to win!"))
...
/datum/mafia_role/obsessed/proc/check_victory(datum/source,datum/mafia_controller/game,lynch)
	...
	if(lynch)
		...
		to_chat(body, span_userdanger("I have failed your objective to lynch [obsession.body]!"))
